...
test.describe("Watchlist Feature", () => {
  ...
  test("should display empty watchlist for new user", async ({ page }) => {
    ...
    await expect(page.locator('text="Jūs dar nesekate jokių Seimo narių"')).toBeVisible();
  });

  test("should allow user to follow an MP from profile page", async ({ page }) => {
    ...
    const followButton = page.locator('button[aria-label*="Sekti"], button[aria-label*="Nebesekti"]').first();
    ...
    await expect(followButton).toHaveAttribute("aria-label", /Nebesekti/i, { timeout: 2000 });
    ...
    if (await watchlistWidget.isVisible()) {
      ...
      const emptyMessage = page.locator('text="Jūs dar nesekate jokių Seimo narių"');
      await expect(emptyMessage).not.toBeVisible({ timeout: 2000 });
    }
  });

  test("should show error when accessing watchlist without authentication", async ({ page }) => {
    // Logout first
    await logout(page);
