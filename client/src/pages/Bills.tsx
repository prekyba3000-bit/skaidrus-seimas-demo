      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-10">
        {isLoading ? (
          Array.from({ length: 9 }).map((_, i) => (
            <div key={i} className="h-48 bg-surface-dark rounded-xl animate-pulse border border-surface-border" />
          ))
        ) : (
          filteredBills.map((bill) => {
            const config = statusConfig[bill.status as keyof typeof statusConfig] || statusConfig.proposed;
            return (
              <Link key={bill.id} href={`/bills/${bill.id}`}>
                <div className="group bg-surface-dark border border-surface-border rounded-xl p-6 hover:shadow-lg hover:border-primary/50 transition-all cursor-pointer flex flex-col h-full">
                  <div className="flex justify-between items-start mb-4">
                    <Badge variant="outline" className="text-[10px] uppercase font-bold tracking-widest border-surface-border text-[#92adc9]">
                      {bill.category}
                    </Badge>
                    <Badge className={`font-black text-[9px] uppercase tracking-widest border-2 ${config.color}`}>
                      {config.label}
                    </Badge>
                  </div>
                  ...
                  <div className="flex justify-between items-center py-2 border-t border-surface-border mt-auto">
                    <span className="text-primary text-[10px] font-black uppercase tracking-widest">Skaityti daugiau</span>
