        ...
        <div className="h-[400px] flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-[#92adc9] mb-6">
              Nurodytas Seimo narys neegzistuoja arba buvo pašalintas
            </p>
    ...
    <DashboardLayout title={`Profilis: ${mp.name}`}>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="bg-[#233648] border border-surface-border rounded-lg px-3 py-1.5 flex items-center gap-2">
            ...
            <span className="text-[#92adc9] text-xs font-medium uppercase tracking-widest">
              ID: {mp.seimasId}
            </span>
          </div>
        </div>
      ...
      <div className="relative glass-panel rounded-2xl overflow-hidden p-4 sm:p-6 md:p-8 lg:p-10 group mb-6">
        ...
        <div className="absolute inset-0 pointer-events-none opacity-20 group-hover:opacity-25 transition-opacity duration-700">
          <svg
            className="absolute top-[-50px] left-1/2 -translate-x-1/2 w-full max-w-[800px] h-[400px] sm:h-[500px] md:h-[600px] text-primary"
            preserveAspectRatio="none"
            viewBox="0 0 100 100"
          >
            {/* Triangle Roof Shape */}
            <path
              d="M50 10 L10 60 H90 Z"
              fill="none"
              stroke="currentColor"
              strokeWidth="0.5"
            />
            <path
              d="M50 15 L15 58 H85 Z"
              fill="none"
              stroke="currentColor"
              strokeWidth="0.2"
            />
        ...
        <div className="relative z-10 flex flex-col md:flex-row gap-4 sm:gap-6 md:gap-8 items-center md:items-start">
          ...
          <div className="relative shrink-0">
            <div className="size-24 sm:size-28 md:size-32 rounded-2xl overflow-hidden border border-primary/40 shadow-[0_0_20px_rgba(245,159,10,0.25)] relative">
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-left flex-1">
            ...
            <p className="text-gray-400 text-lg mb-4 max-w-2xl">
              Seimo narys <span className="text-primary mx-2">•</span>{" "}
              {mp.party || "Nepartinis"}
            </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 md:gap-8 border-t md:border-t-0 md:border-l border-[#1f3a33] pt-4 sm:pt-6 md:pt-0 md:pl-8 mt-2 md:mt-0 justify-center md:justify-start w-full md:w-auto">
            <div className="text-center md:text-left">
              ...
              <p className="text-xl sm:text-2xl font-bold text-primary">
                {stats?.votingAttendance
                  ? parseFloat(stats.votingAttendance).toFixed(0)
                  : 0}
                %
              </p>
            </div>
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-xl sm:text-2xl font-bold text-white">
                {stats?.accountabilityScore
                  ? parseFloat(stats.accountabilityScore).toFixed(0)
                  : 0}
              ...
              >
                <defs>
                  ...
                  >
                    <stop
                      offset="0%"
                      style={{ stopColor: "#f59f0a", stopOpacity: 1 }}
                    />
                    <stop
                      offset="100%"
                      style={{ stopColor: "#f59f0a", stopOpacity: 0.1 }}
                    />
                  </linearGradient>
                </defs>
              ...
              <div className="space-y-12 relative z-10">
                {votesData && votesData.length > 0 ? (
                  votesData.map((item, idx) => {
                    ...
                    const isFirst = idx === 0;
                    const votedAt = item.vote.votedAt
                      ? new Date(item.vote.votedAt)
                      : null;
                    const relativeTime = votedAt
                      ? formatDistanceToNow(votedAt, { addSuffix: true })
                      : null;
                    ...
                    return (
                      <div key={idx} className="relative pl-8 md:pl-12 group">
                        {/* Timeline Node */}
                        <div
                          className={`absolute left-0 md:left-0 top-1 size-7 bg-[#0d261f] rounded-full flex items-center justify-center z-20 transition-all ${
                            isFirst
                              ? `border-2 ${voteConfig.borderColor} shadow-[0_0_10px_rgba(245,159,10,0.5)]`
                              : `border border-slate-600 group-hover:border-2`
                          } ${
                            !isFirst &&
                            voteConfig.borderColor === "border-primary"
                              ? "group-hover:border-primary"
                              : !isFirst &&
                                  voteConfig.borderColor === "border-rose-500"
                                ? "group-hover:border-rose-500"
                                : !isFirst &&
                                    voteConfig.borderColor ===
                                      "border-amber-500"
                        ...
                        >
                          {isFirst ? (
                            <div
                              className={`size-2 rounded-full ${voteConfig.bgColor === "bg-primary" ? "bg-primary" : voteConfig.bgColor === "bg-rose-500" ? "bg-rose-500" : voteConfig.bgColor === "bg-amber-500" ? "bg-amber-500" : "bg-slate-600"}`}
                            />
                          ) : (
                            <VoteIcon
                              className={`w-3.5 h-3.5 ${voteConfig.color} group-hover:scale-110 transition-transform`}
                            />
                          )}
                        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="space-y-6">
            <div className="glass-panel p-6 rounded-xl flex flex-col gap-4">
              ...
              <div className="flex items-end justify-between">
                <div className="flex flex-col">
                  ...
                  <span className="text-gray-400 text-xs uppercase font-bold tracking-widest mt-1">
                    Pateikta projektų
                  </span>
                </div>
              ...
              <Progress
                value={
                  ((stats?.billsPassed || 0) / (stats?.billsProposed || 1)) *
                  100
                }
            ...
            <div className="glass-panel p-6 rounded-xl flex flex-col gap-4">
              ...
              <div className="flex items-end justify-between">
                <div className="flex flex-col">
                  <span className="text-4xl font-black text-white">
                    {stats?.votingAttendance
                      ? parseFloat(stats.votingAttendance).toFixed(1)
                      : 0}
                    %
                  </span>
                  <span className="text-gray-400 text-xs uppercase font-bold tracking-widest mt-1">
              ...
              <Progress
                value={
                  stats?.votingAttendance
                    ? parseFloat(stats.votingAttendance)
                    : 0
                }
                className="h-1.5 bg-[#233648]"
            ...
            <div className="glass-panel rounded-xl p-6 flex flex-col gap-6">
                ...
                <div className="relative w-16 h-16 shrink-0 bg-transparent">
                  <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path
                      ...
                      fill="none"
                      stroke="currentColor"
                      strokeDasharray={`${stats?.partyLoyalty ? parseFloat(stats.partyLoyalty) : 0}, 100`}
                      strokeLinecap="round"
                      strokeWidth="3"
                    />
                  </svg>
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-white text-lg font-black">
                      {stats?.partyLoyalty
                        ? parseFloat(stats.partyLoyalty).toFixed(0)
                        : 0}
                      %
                    </span>
                  </div>
              ...
              <p className="text-gray-400 text-sm leading-relaxed">
                Lojalumo balas skaičiuojamas pagal tai, kaip narys balsuoja
                lyginant su frakcijos dauguma. Šiuo metu {mp.name} balsavimai
                sutampa su partijos linija{" "}
                <span className="text-white font-bold">
                  {stats?.partyLoyalty
                    ? parseFloat(stats.partyLoyalty).toFixed(1)
                    : 0}
                  %
                </span>{" "}
                atvejų.
          ...
          <div className="glass-panel p-6 rounded-xl">
            ...
            <p className="text-gray-400 text-sm leading-relaxed line-clamp-6">
              {mp.biography ||
                "Biografija šiuo metu nepasiekiama. Duomenys sinchronizuojami iš Seimo API."}
            </p>
          </div>
        </div>
      </div>
