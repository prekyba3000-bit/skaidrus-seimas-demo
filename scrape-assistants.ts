...
async function scrapeAssistants() {
  ...
  await page.goto(
    "https://www.lrs.lt/sip/portal.show?p_r=35381&p_k=1&p_a=31&p_asm_id=47220"
  );
  ...
  for (let i = 0; i < mpOptions.length; i++) {
    ...
    console.log(
      `[${i + 1}/${mpOptions.length}] Extracting assistants for ${mp.name}...`
    );
  ...
  console.log(
    `✓ Extraction complete. Saved ${allAssistants.length} assistants to mp_assistants.json`
  );
