...
async function scrapeAssistants() {
  ...
  for (let i = 0; i < mpOptions.length; i++) {
    ...
    try {
      // Use direct navigation to avoid context destruction issues with selectOption
      await page.goto(`https://www.lrs.lt/sip/portal.show?p_r=35381&p_k=1&p_a=31&p_asm_id=${mp.id}`);
