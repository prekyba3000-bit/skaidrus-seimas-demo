 ...
 */
async function scrapeBills(job: Job<ScrapeBillsJobData>): Promise<{ processed: number; skipped: number; total: number }> {
  const { limit, force } = job.data;
  logger.info({ jobId: job.id, limit, force }, "[Worker] Starting bills scrape job");
  ...
  try {
    ...
    try {
      const pfButtons = await page.$$('button[id*="search"], button[id*="submit"], input[id*="search"], input[id*="submit"]');
      if (pfButtons.length > 0) {
        await pfButtons[0].click();
        await page.waitForTimeout(4000);
      }
    } catch (e) {
      logger.warn({ jobId: job.id, err: e }, "[Worker] Search button click failed, continuing...");
    }

    // Wait for results table
    const tableSelector = "#searchCompositeComponent\\:contentForm\\:resultsTable_data";
    await page.waitForSelector(tableSelector, { timeout: 20000 });
    await page.waitForTimeout(3000);

    const rows = await page.$$(tableSelector + " tr");
    logger.info({ jobId: job.id, rowCount: rows.length }, "[Worker] Found rows in table");
    ...
    for (let i = 0; i < maxRows; i++) {
      ...
      try {
        ...
        let dateStr = "";
        try {
          dateStr = await row.$eval("td:nth-child(6) span.noWrap", (el) => (el.textContent ?? "").trim());
        } catch {
          try {
            dateStr = await row.$eval("td:nth-child(6)", (el) => (el.textContent ?? "").trim());
          } catch {
        ...
        let status = "";
        try {
          status = await row.$eval("td:nth-child(7)", (el) => (el.textContent ?? "").trim());
        } catch {
        ...
        logger.error({ jobId: job.id, rowIndex: i, err }, "[Worker] Error processing row");
        skippedCount++;
      }
    }
    ...
    const result = { processed: processedCount, skipped: skippedCount, total: maxRows };
    logger.info({ jobId: job.id, ...result }, "[Worker] Bills scrape job completed");
    return result;

  } catch (error) {
    await browser.close();
    await client.end();
    logger.error({ jobId: job.id, err: error }, "[Worker] Bills scrape job failed");
    throw error;
  }
}
...
export function startScraperWorker(): Worker {
  ...
  const worker = new Worker<ScrapeBillsJobData>(
    ...
    },
    {
      ...
      maxStalledCountResetInterval: 60000, // 1 minute
    }
  );

  // Event handlers
  worker.on("completed", (job) => {
    logger.info({ jobId: job?.id, result: job?.returnvalue }, "[Worker] Job completed");
  });

  worker.on("failed", (job, err) => {
    logger.error({ jobId: job?.id, attemptsMade: job?.attemptsMade, err }, "[Worker] Job failed");
  });
