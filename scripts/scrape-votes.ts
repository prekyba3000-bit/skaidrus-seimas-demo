...
async function scrapeVotes() {
  ...
  // Handle Cookies
  try {
    const acceptCookies = page.getByRole('button', { name: 'Patvirtinti visus' });
  ...
  page.on('response', async response => {
      ...
      if (url.includes('portal.show') && !url.includes('.css') && !url.includes('.js') && !url.includes('.png')) {
  ...
  } catch(e) { console.warn("[Scrape] Failed to inspect custom session dropdown:", e); }
  ...
  try {
      ...
      if (count > 0) {
          ...
          const responsePromise = page.waitForResponse(resp => resp.url().includes('p_fakt_pos_id') && resp.status() === 200, { timeout: 10000 });
          ...
          try {
              ...
              if (text.includes('Balsavo')) console.log("[Scrape] Response contains 'Balsavo'");
              else console.log("[Scrape] Response DOES NOT contain 'Balsavo'");
          } catch(e) { console.log("Response capture failed", e); }
          
          await page.waitForTimeout(3000); 
      }
  } catch(e) { console.error("Sitting selection failed", e); }
  
  // Dump text content
  const content = await page.evaluate(() => document.body.innerText);
  console.log(`[Scrape] Body Text length: ${content.length}`);
  if (content.includes("Balsavo")) console.log("[Scrape] Page has 'Balsavo' visible.");
  ...
  try {
     ...
     await frameElement.locator('body').waitFor({ state: 'attached', timeout: 5000 });
     ...
     console.log(`[Scrape] Extracted ${voteData.length} valid vote rows from iframe.`);
     if (voteData.length > 0) {
      ...
      console.log("[Scrape] Iframe access failed or timed out:", e.message);
      
      // Fallback: Check main page
      const contentRows = await page.evaluate(() => {
          const elements = Array.from(document.querySelectorAll('.bals-rez-row, .list-row, tr'));
          return elements.map(e => (e as HTMLElement).innerText.replace(/\s+/g, ' ').trim())
