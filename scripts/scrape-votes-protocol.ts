...
async function scrapeProtocolVotes() {
  ...
  const latestSession = sessions.find((s: any) => s["@_sesijos_id"] == targetSessionId) || sessions[0];
  console.log(`[Protocol] Selected Session: ${latestSession["@_pavadinimas"]} (ID: ${latestSession["@_sesijos_id"]})`);
  ...
  const sittingsWithDoc = sittings.filter((s: any) => 
  ...
  let targetUrl = "";
  if (targetSitting.Stenograma && targetSitting.Stenograma["@_stenogramos_nuoroda"]) {
  ...
  try {
      ...
      const textTab = page.locator('a, div, li, span').filter({ hasText: /^Dokumento tekstas$/ }).first();
      
      // Also look for class 'parsed-tab-header' which is common in e-seimas
      const specificTab = page.locator('.parsed-tab-header:has-text("Dokumento tekstas")');
      ...
      if (clicked) {
          console.log("[Protocol] Clicked 'Dokumento tekstas' tab. Waiting for content...");
          await page.waitForTimeout(3000); 
      } else {
          console.log("[Protocol] 'Dokumento tekstas' tab not found/visible.");
      }
  } catch(e) { console.log("Tab switching error", e); }

  // Check for Voting links/data
  // In the text, voting results are often links with text "Balsavimo rezultatai"
  const voteLinks = await page.getByRole('link', { name: /Balsavimo rezultatai|duomenys/i }).all();
  ...
  for (const link of voteLinks) {
      ...
      if (href) {
           ...
           try {
               ...
               const xmlLink = await votePage.getByRole('link', { name: /XML/i }).first();
           ...
           } catch(e) { console.log("Failed to visit vote link", e); }
      }
  }
