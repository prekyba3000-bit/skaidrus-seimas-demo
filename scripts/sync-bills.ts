...
async function syncBills() {
  ...
  try {
    ...
    try {
      const formButtons = await page.$$('form button, form input[type="submit"]');
      if (formButtons.length > 0) {
        console.log(`[Bills] Found ${formButtons.length} form buttons, clicking first one...`);
    ...
    if (!tableExists) {
      console.log("[Bills] WARNING: Table selector not found, checking page content...");
      const pageContent = await page.content();
      console.log(`[Bills] Page content length: ${pageContent.length}`);
      // Look for alternative table selectors
      const altTables = await page.$$('table, [id*="resultsTable"], [id*="table"]');
    ...
    if (rows.length > 0) {
      ...
      console.log(`[Bills] First row content preview: "${firstRowText?.substring(0, 200)}"`);
    }
    ...
    await db.transaction(async (tx) => {
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!seimasId) {
          console.log(`[Bills] Skipping row ${i + 1} (could not extract ID from href: ${href})`);
          skippedCount++;
          continue;
        }
        ...
        try {
          ...
          console.log(`[Bills] Row ${i + 1}: Could not extract registration number`);
        }
        ...
        if (!dateStr) {
          ...
          const truncatedTitle = billData.title.length > 500 
          ...
          processedCount++;
          const titlePreview = truncatedTitle.length > 60 
            ? truncatedTitle.substring(0, 60) + "..." 
            : truncatedTitle;
          logger.debug({ billId: seimasId, title: titlePreview }, "Processed bill");
        } catch (err: any) {
    ...
    const status = failedCount > 0 && processedCount > 0 ? "partial" : processedCount > 0 ? "success" : "failed";
    await updateSystemStatus(db, "bills_sync", status, processedCount, failedCount);

    logger.info(
      { processed: processedCount, skipped: skippedCount, failed: failedCount, total: rows.length },
